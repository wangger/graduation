...
#post_detail文件
detail_path = 'moc_post_detail'
detail_data =  []

for i in range(2):
    detail_name = detail_path + '/' + '00000'+str(i)+'_0'    
    with open(detail_name,'r',encoding = 'utf-8') as f:
        temp_info = f.readlines()
        delim = '\x01'
        for line in temp_info: 
            detail_data.append(line.strip().split(delim))
detail_column = ['id','gmt_create','gmt_modified','content']
detail_data = get_dataframe(detail_data, detail_column)

print(detail_data.dtypes)

#merge(inner)
post_data = pd.merge(post_data, detail_data, left_on = 'id', right_on = 'id')
post_extract_column = ['poster_id','post_time','content']
...
post_data['post_len'] = post_data['content'].map(lambda x : len(x))
post_data['post_len'] = post_data['post_len'].astype('int')
...
sum_tag = 'post_len'
user_feature = Usersplit_sum(timeseries,user_tag,sum_tag)
